...
// Categorías pizza (solo estas consumen plaza de franja)
function basati_pizza_cats(): array {
    return [ 'pizza', 'pizza-erdiak' ];
}

// Cuenta artículos de pizza en un pedido
function basati_count_pizza_items( WC_Order $order ): int {
    $cats  = basati_pizza_cats();
    $count = 0;
    foreach ( $order->get_items() as $item ) {
        $terms = wp_get_post_terms( $item['product_id'], 'product_cat', [ 'fields' => 'slugs' ] );
        if ( ! empty( array_intersect( $cats, is_array( $terms ) ? $terms : [] ) ) ) {
...
// Total de pizzas en una franja (soporta formato antiguo int y nuevo array)
function basati_slot_online_total( $slot_data ): int {
    if ( is_numeric( $slot_data ) ) return (int) $slot_data;
    if ( ! is_array( $slot_data ) )  return 0;
    return (int) array_sum( array_column( $slot_data, 'qty' ) );
}
...
function basati_day_slots( string $date, string $type ): array {
    ...
    foreach ( basati_generate_slots() as $slot ) {
        ...
        $slot_orders = is_array( $on[ $slot ] ?? null ) ? $on[ $slot ] : [];
        $o      = min( $max, basati_slot_online_total( $on[ $slot ] ?? 0 ) );
        $checks = $raw[ $slot ];
...
add_action( 'woocommerce_checkout_order_created', function( $order ) {
    ...
    $pizza_qty = basati_count_pizza_items( $order );
    if ( $pizza_qty > 0 ) {
        if ( ! is_array( $data[ $slot ] ?? null ) ) $data[ $slot ] = [];
        $data[ $slot ][] = [ 'id' => $order->get_id(), 'qty' => $pizza_qty ];
    }
...
function basati_remove_slot_items( int $order_id ): void {
    ...
    $data = (array) get_option( $key, [] );
    if ( is_array( $data[ $slot ] ?? null ) ) {
        $data[ $slot ] = array_values( array_filter( $data[ $slot ], fn( $o ) => (int) ( $o['id'] ?? 0 ) !== $order_id ) );
    } else {
        $pizza_qty     = basati_count_pizza_items( $order );
        $data[ $slot ] = max( 0, (int) ( $data[ $slot ] ?? 0 ) - $pizza_qty );
    }
    update_option( $key, $data, false );
}
...
add_filter( 'woocommerce_add_to_cart_validation', function( $passed, $product_id, $quantity ) {
    ...
    if ( $type && $slot ) {
        // Solo las pizzas consumen capacidad de franja
        $pizza_cats   = basati_pizza_cats();
        $product_cats = wp_get_post_terms( $product_id, 'product_cat', [ 'fields' => 'slugs' ] );
        $is_pizza     = ! empty( array_intersect( $pizza_cats, is_array( $product_cats ) ? $product_cats : [] ) );
        if ( $is_pizza ) {
            $pizza_in_cart = 0;
            if ( WC()->cart ) {
                foreach ( WC()->cart->get_cart() as $ci ) {
                    $ct = wp_get_post_terms( $ci['product_id'], 'product_cat', [ 'fields' => 'slugs' ] );
                    if ( ! empty( array_intersect( $pizza_cats, is_array( $ct ) ? $ct : [] ) ) ) {
                        $pizza_in_cart += (int) $ci['quantity'];
                    }
                }
            }
            $date = current_time( 'Y-m-d' );
            if ( ! basati_slot_can_accept( $date, $type, $slot, $pizza_in_cart + $quantity ) ) {
                $slots = basati_day_slots( $date, $type );
                $free  = max( 0, ( $slots[ $slot ]['available'] ?? 0 ) - $pizza_in_cart );
                wc_add_notice(
                    $free > 0
                        ? "Solo quedan {$free} pizzas libres en la franja {$slot}."
...
add_action( 'woocommerce_checkout_process', function() {
    ...
    $date     = current_time( 'Y-m-d' );
    if ( $pizza_in_cart > 0 && ! basati_slot_can_accept( $date, $type, $slot, $pizza_in_cart ) ) {
...
function basati_render_control_page(): void {
    ...
    function basati_render_slot_table( array $slots, string $type, string $label, string $nonce_manual ): void {
        ...
        <div class="bc-slot-table">
            ...
            <table class="bc-slot-tbl">
                ...
                data-orders-json="<?php echo esc_attr( wp_json_encode( $d['orders'] ?? [] ) ); ?>">

                            <?php

                            $pos = 0;

                            foreach ( ( $d['orders'] ?? [] ) as $web_order ) :
                                $oid      = (int) ( $web_order['id'] ?? 0 );
                                $oqty     = (int) ( $web_order['qty'] ?? 0 );
                                $order_url = admin_url( 'admin.php?page=wc-orders&action=edit&id=' . $oid );
                                echo '<span class="bc-slot-order-group">';
                                for ( $wi = 0; $wi < $oqty; $wi++ ) :
                                    if ( $pos > 0 && $pos % 5 === 0 ) : ?><span class="bc-check-sep"></span><?php endif;
                                    $pos++;
                            ?>
                                    <input type="checkbox" class="bc-slot-check bc-slot-check-web" checked disabled data-order="<?php echo $oid; ?>" title="Comanda #<?php echo $oid; ?>">
                                <?php endfor;
                                echo '<a href="' . esc_url( $order_url ) . '" target="_blank" class="bc-order-link">#' . $oid . '</a>';
                                echo '</span>';
                            endforeach;
    ...
    <p style="font-size:.78rem;color:#888;margin:.8rem 0 0;display:flex;gap:1.2rem;align-items:center;flex-wrap:wrap">
        ...
        <span style="display:flex;align-items:center;gap:.3rem"><input type="checkbox" checked disabled style="width:14px;height:14px;accent-color:#e67e22;margin:0"> Pizza web (clic # = ver comanda)</span>
    ...
    (function($){
        ...
        function checksHtml(d) {
            ...
            orders.forEach(function(o) {
                ...
                var orderUrl = adminBase + 'admin.php?page=wc-orders&action=edit&id=' + oid;
                html += '<span class="bc-slot-order-group">';
                for (var wi = 0; wi < oqty; wi++) {
                    if (pos > 0 && pos % 5 === 0) html += '<span class="bc-check-sep"></span>';
                    html += '<input type="checkbox" class="bc-slot-check bc-slot-check-web" checked disabled data-order="' + oid + '" title="Comanda #' + oid + '">';
                    pos++;
                }
                html += '<a href="' + orderUrl + '" target="_blank" class="bc-order-link">#' + oid + '</a>';
                html += '</span>';
            });
